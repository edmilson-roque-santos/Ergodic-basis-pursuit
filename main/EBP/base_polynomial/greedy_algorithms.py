"""
Collection of algorithms to reconstruct network from data.

Created on Tue Jun  8 16:09:34 2021
...
from . import pre_settings as pre_set 
...
def completing_coeff_matrix(id_node, x_eps_can, x_eps_matrix, params_node, params,
    ...
    id_nonzero_basis = np.where(np.absolute(x_eps_matrix[:, id_node]) >= threshold)[0]
    power_indices = params['power_indices'][id_nonzero_basis, :]
    for l in range(power_indices.shape[0]):
        ...
        for id_conn in entries_ones[mask_nodes_identifier]:
            ...
            mask_index_identifier = params['power_indices'] == exp_vector_temp
            ...
            x_eps_matrix[mask_index_identifier, id_conn] = x_eps_matrix[id_nonzero_basis[l], id_node] #x_eps_can
...
def B_eps_algorithm(B, PHI_incluster, params, 
    ...
    for id_node in id_trial:
        ...
        if params_['use_orthonormal']:
            R = params_['R']
            full_vec = R @ min_l2_sol#l2_coeff_matrix
...
def MS_dict(X_t_, model_selection_dict, params, library_method = polb.library_matrix):
    '''
...
def GR_algorithm(X_t_, initial_partition, params, tol = tolp,
                 ...
                 solver_default = cp.ECOS, 
                 model_selection_dict = MS_dict_default,
                 library_method = polb.library_matrix):
    '''
    ...
    if params_['use_orthonormal'] and params['build_from_reduced_basis']:
        params_ = pre_set.set_orthnormfunc(params['orthnorm_func_filename'], params_)
    ...
    if not card_comp_set == params['nodelist'].shape[0]:
